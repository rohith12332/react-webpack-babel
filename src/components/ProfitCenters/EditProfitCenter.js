...
class EditProfitCenter extends React.Component {
    constructor(props) {
      super(props);
      this.state = {
        msgFailure: "",
    ...
    onSubmit(event) {
      ...
      if (isValid) {
        ...
        const request = new Request(`${process.env.API_HOST}/ManageProfitCenters.svc//UpdateProfitCenter/json`, {
          method: 'POST',
          headers: new Headers({
    ...
    componentDidMount() {
      this.getSingleProfitCenter();
      this.getAllProductGroups();
      this.getTaxGroups();
    }

    getSingleProfitCenter() {
      var that = this;
      var reqQuery = {};
      var profitcenterId = this.props.location.pathname.split('/')[2];
      reqQuery['userdetails'] = getUserDetails();
      reqQuery['ProfitCenterID'] = profitcenterId;
      const request = new Request(`${process.env.API_HOST}/ManageProfitCenters.svc/GetSingleProfitCenter/json`, {
        method: 'POST',
        headers: new Headers({
          'Content-Type': 'application/json'
        }),
      ...
      fetch(request).then(function(response) {
        ...
        for (var j = 0; j < data.profitcenter.taxableproductgroups.length; j++) {
          selectedproductgroups.push(data.profitcenter.taxableproductgroups[j].id);
        }
        that.setState({
          profitcenter: data.profitcenter,
          SelectedTaxGroupId: data.profitcenter.taxgroupid,
...
render()
  {
    ...
    var currentStore = window.sessionStorage.getItem("currentstorename");
     var that = this;
  	 const {pageHead,msgFailure,msgSuccess,taxGroupList,ProductGroups,productGroupsId} = this.state;
     const format = 'h:mm a';
        ...
        <DefaultLayout>
            ...
            {msgSuccess}</div>}
                    ...
                    <div className="widget widget-small">
                      <form action="#" className="">
                        <div className={classnames( 'form-group', { error: !!this.state.errors.name})}>
                          <TextInput type="text" name="name" label="Profit Center Name" value={this.state.profitcenter.name} defaultValue={this.state.profitcenter.name} placeholder="" required="*" onChange={this.onChange} className={classnames( 'form-control', { error: !!this.state.errors.name})} /> <span>{this.state.errors.name}</span> </div>
                        <div className="form-group"> <label>Tax Group<span className="required"></span></label>
                          <Select name="taxGroup" value={this.state.SelectedTaxGroupId} options={taxGroup} onChange={this.updateCountry} /> </div> {/*
                        <div className="form-group"> <label>Surcharge Group<span className="required"></span></label>
                          <Select name="surchargeGroup" /> </div>*/}
                        <div className="row">
                          <div className="col-sm-12">
                            <div className="tabbable-panel profit-center-tabbing">
                              <div className="tabbable-line">
                                <ul className="nav nav-tabs ">
                                  ...
                                  <li> <a href="#taxconfig" data-toggle="tab">Product Groups to Surcharge</a> </li>*/} </ul>
                                            ...
                                            <div className="am-scroller nano">
                                              <div className="content nano-content">
                                                <div className="product-box-list">
                                                  <ul> {prdtGroup.map((prdtGroup) => prdtGroup.label )} </ul>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
              ...
              <div className="col-sm-12 form-bot-butn-wrap">
                <div className="form-bot-butns round-btns"> <button type="submit" className="btn btn-primary" onClick={this.onSubmit}>Save</button> <button type="reset" className="btn btn-default" onClick={this.onReset}>Cancel</button> </div>
              </div>
                  </div>
                </div>
              </div>
...
export default EditProfitCenter;
